...
static void gen_PrintDistanceToTarget()
{
	xor_PrintUInt24Tenths(gen_distanceToTarget);
	xor_Print(" Light Years");
}

static void gen_PrintSelectionOnMap()
{
//	xor_SetCursorPos(0, xor_textRows - 1);
	xor_SetCursorPos(0, xor_textRows - 2);
...
bool gen_ChangeSystem()
{
	...
	struct ship_t* newPlanet = NewShip(PLANET,
...
static void gen_RotateBytes(unsigned char *start)
{
