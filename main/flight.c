...
static const VMUINT16 elite_palette[] =
{
    0x0000, /* black  */
    0xffff, /* white  */
    0xffe0, /* yellow */
    ...
    const unsigned char *sprite)
{
    ...
    const unsigned char *pixels = sprite + 2;

    for (int py = 0; py < h; py++)
    {
        for (int px = 0; px < w; px++)
        {
            unsigned char idx = pixels[py * w + px];
...
//{
//    struct vector_t v = normalize((stationSoi ? station : planet).position);
...
//    plat_FillRect(
//        COMPASS_HCENTER + v.x / COMPASS_SCALE,
//        COMPASS_VCENTER + v.y / COMPASS_SCALE,
//        2,
//        1,
//        color);
////    plat_FillRect(
...
{
    const struct vector_t target = (stationSoi ? station : planet).position;
    const unsigned int mag = magnitude(target);

    if (mag == 0)
    {
        plat_FillRect(COMPASS_HCENTER - 1, COMPASS_VCENTER - 1, 3, 3, PLT_COLOR_YELLOW);
        return;
    }
    ...
    const VMUINT16 color = (v.z >= 0) ? PLT_COLOR_YELLOW : PLT_COLOR_GREEN;
...
static void drawRadarDot(const struct ship_t *ship)
{
    ...
    int x  = RADAR_HCENTER +
             ship->position.x / RADAR_XSCALE;

    int y  = RADAR_VCENTER -
             ship->position.z / RADAR_ZSCALE;

    int dy = ship->position.y /
             RADAR_YSCALE;
    ...
    plat_Line(x, y, x, y + dy, color);

    plat_SetPixel(x,     y + dy, color);
    plat_SetPixel(x - 1, y + dy, color);
}
...
void drawDashboard(void)
{
    ...
    plat_FillRect(
        DASH_HOFFSET + 20,
        DASH_VOFFSET + 2,
        player_fore_shield / 8 + 1,
        3,
        player_fore_shield < 64
            ? PLT_COLOR_RED
            : PLT_COLOR_YELLOW);
    ...
    plat_FillRect(
        DASH_HOFFSET + 20,
        DASH_VOFFSET + 10,
        player_aft_shield / 8 + 1,
        3,
        player_aft_shield < 64
            ? PLT_COLOR_RED
            : PLT_COLOR_YELLOW);
