...
//    timer_id = vm_create_timer_ex(9000, exit_timer);
   ...
   if (player_dead)
    ...
    for (unsigned char i = 0; i < NUM_PLAYER_DEATH_CANS; i++)
    {
        struct ship_t* can = NewShip(BP_CANISTER,
                                     (struct vector_t){ 0, 0, 1024 },
                                     Matrix(256,0,0, 0,256,0, 0,0,256));
        can->speed       = player_speed / 4;
        can->pitch       = 127;
        can->roll        = 127;
        can->toExplode = true;
//        can->isExploding = true;
