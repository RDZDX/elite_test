...
//	xor_SetCursorPos(0, NUM_UPGRADES + 8);
        xor_SetCursorPos(0, NUM_UPGRADES + 6);
	xor_Print("Cash: ");
...
void upg_DisplayEquipment()
{
	...
	// always print equipped lasers (fix: compare player_lasers[0], not array pointer)
	xor_Print(upg_displayNames[player_lasers[0] == PULSE   ? 3
							 : player_lasers[0] == BEAM    ? 4
							 : player_lasers[0] == MINING  ? 11
							 : 12]);
