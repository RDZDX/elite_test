...
unsigned char mkt_qtyValue = 0;
unsigned char mkt_qtyGood = 0;
bool mkt_qtyBuy = true;
...
bool mkt_BuyAmount(unsigned char goodIndex,
                   unsigned char quantity)
{
    if (quantity > mkt_localQuantities[goodIndex])
        return false;

    if (tradeGoods[goodIndex].unitSymbol == 't')
    {
        if (quantity > player_cargo_space)
            return false;
    }
    else
    {
        if (inventory[goodIndex] + quantity > 200)
            return false;
    }

    unsigned int price =
        (unsigned int)mkt_localPrices[goodIndex] *
        quantity * 4;
...
bool mkt_SellAmount(unsigned char goodIndex,
                    unsigned char quantity)
{
    if (quantity > inventory[goodIndex])
        return false;
...
unsigned char mkt_GetInventoryRow(unsigned char goodIndex)
{
    ...
    for (unsigned char i = 0; i < goodIndex; i++)
    {
        if (inventory[i] > 0)
            row++;
    }
